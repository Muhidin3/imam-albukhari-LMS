...
import { Menu, X, GraduationCap, ChevronDown, LogIn } from 'lucide-react';
...
export default function PublicNavbar() {
    ...
    return (
        <nav className="fixed top-0 left-0 right-0 z-50 bg-white/60 backdrop-blur-2xl shadow-lg  mx-2 mt-2 rounded-lg">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex items-center justify-between h-16">
                    ...
                    <div className="hidden md:flex items-center gap-1">
                        <Link href="/" className="text-orange-900 hover:text-orange-400 px-4 py-2 text-sm font-medium transition-colors rounded-lg hover:bg-white/5">
                            Home
                        </Link>

                        <div className="relative" onMouseEnter={() => setProgramsOpen(true)} onMouseLeave={() => setProgramsOpen(false)}>
                            <Link href="/programs" className="text-orange-900 hover:text-orange-400 px-4 py-2 text-sm font-medium transition-colors rounded-lg hover:bg-white/5 flex items-center gap-1">
                                Programs <ChevronDown className="w-3 h-3" />
                            </Link>
                            {programsOpen && (
                                <div className="absolute top-full left-0 mt-1 w-64 bg-[#3A1700] border border-white/10 rounded-xl shadow-2xl p-2 animate-fade-in-up">
                        ...
                        <Link href="/about" className="text-orange-900 hover:text-orange-400 px-4 py-2 text-sm font-medium transition-colors rounded-lg hover:bg-white/5">
                            About
                        </Link>
                    </div>
                    ...
                    <div className="hidden md:flex items-center gap-3">
                        <Link href="/login" className="text-orange-900 hover:text-orange-400 px-4 py-2 text-sm font-medium transition-colors flex items-center gap-2">
